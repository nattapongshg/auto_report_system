...
This file was exported automatically from Metabase and includes the latest workbook generated by the report template.
...
function renderRecipients() {
  recipientListNode.innerHTML = state.recipients.map((recipient, index) => `
    <div class="recipient-chip">
      <span>${recipient}</span>
      <button type="button" class="chip-remove" data-recipient-index="${index}">×</button>
    </div>
  `).join('')
}
...
function buildScheduleSummary() {
  ...
  if (state.frequency === 'weekly') {
    ...
    return `Weekly on ${weekdayLabels || 'selected days'} from ${state.startDate} at ${state.runTime} (${state.timezone})`
  }
...
function nextRuns() {
  ...
  if (state.frequency === 'weekly') {
    for (let offset = 0; runs.length < 3 && offset < 21; offset += 1) {
      const candidate = new Date(start)
      candidate.setDate(start.getDate() + offset)
      candidate.setHours(hour, minute, 0, 0)
...
function renderUpcomingRuns() {
  ...
  upcomingRunsNode.innerHTML = runs.map((run, index) => `
    <div class="run-item">
      <div>
...
function resetColumnsForCurrentSource() {
  state.selectedColumns = [...getSource().columns]
}
